...
{
    // Convert xRadon:// to xRadon:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("xRadon://"))
    {
        uri.replace(0, 12, "xRadon:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "xRadon.lnk";
}
...
{
    return GetAutostartDir() / "xRadon.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("xRadon-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  xRadon-qt [" + tr("command-line options") + "]                     " + "\n";
